...
typically define a legal case. If the description aligns with what you understand as a legal case,predict Case Rating, 
Case Type for the description.
...
if __name__ == "__main__":
   # while True:
    ...
    # Input DataFrame
    #input_data = pd.read_excel('SSD Intakes for AI Scrub Sample Data 1.xlsx',nrows=150)
    #df_input_description = pd.DataFrame(input_data)
    
    # File to save results
    #excel_file = 'SSD_results_150_new.xlsx'

    # Create a list to hold results
    #results = []

    # Loop through each description in the DataFrame
    #for index, row in df_input_description.iterrows():
       # query = row['Description']
        ...
        #logging.info('Final result generated: %s', response)
        #print("description:",query,"response:", response)

        # Append the current query and response to the results list
        #results.append({"Description": query, "Response": response})
   ...
   # results_df = pd.DataFrame(results)
    ...
    #print(f"All case descriptions and their results saved to '{excel_file}'.")
    # Load the input data
    input_data = pd.read_excel('SSD Intakes for AI Scrub Sample Data 1.xlsx', nrows=150)
    df_input_description = pd.DataFrame(input_data)

    # File to save results
    excel_file = 'SSD_results_150_updated.xlsx'
    ...
    for index, row in df_input_description.iterrows():
        ...
        # Convert response to a dictionary if it's in JSON format
        if isinstance(response, str):
            response_dict = json.loads(response)
        else:
            response_dict = response

        # Prepare a result row with the expected columns
        result_row = {
            "Description": query,
            "Status": response_dict.get("Status", ""),
            "CaseType": response_dict.get("CaseType", ""),
            "Case_Rating": response_dict.get("Case_Rating", ""),
            "Explanation": response_dict.get("Explanation", "")
            }

        # Append the current row to the results list
